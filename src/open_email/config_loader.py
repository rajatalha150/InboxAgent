...
def load_yaml(path: Path) -> dict:
    """Load a YAML file, creating it with default content if it doesn't exist."""
    path = Path(path)
    if not path.exists():
        logger.warning("Config file not found, creating default: %s", path)
        path.parent.mkdir(parents=True, exist_ok=True)
        default_content = {}
        if "accounts" in path.name:
            default_content = {"accounts": []}
        elif "rules" in path.name:
