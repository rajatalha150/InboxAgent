    ...
    if msg.is_multipart():
        for part in msg.walk():
            ...
            try:
                ...
                if charset == "unknown-8bit":
                    charset = "latin-1" # Fallback for this common error
                decoded = payload.decode(charset, errors="replace")
            except (UnicodeDecodeError, LookupError) as e:
                logger.warning("Charset decode failed (%s), trying fallback: %s", charset, e)
                try:
                    decoded = payload.decode("latin-1", errors="replace")
                except UnicodeDecodeError:
                    logger.error("Fallback charset decode also failed.")
                    continue
            except Exception:
                continue
